...
struct huff_symbol *huff_traverse_lookup(struct huff_symbol *cur_node, int direction)
{
	switch (cur_node->kind) {
		case node: {
...
unsigned char huff_tree_decode_char(struct huff_symbol *head, struct bitfile *bf)
{
	int length = code[0];
	int *direction = code + 1;

	return huff_traverse_lookup(head, direction, length);
}
